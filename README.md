...
- [x] 具有波动性的光也会出现这种效应：多普勒-斐索效应
  - 光波频率的变化使人感觉到是颜色的变化
    - 如果恒星远离我们而去，则光的谱线就向红光方向移动，称为 `红移` ；
    - 如果恒星朝向我们运动，光的谱线就向紫光方向移动，称为 `蓝移` 。
  - 法国物理学家斐索(1819-1896)于1848年独立地对来自恒星的波长偏移做了解释，指出了利用这种效应可以测量恒星相对速度。
...
- [x] 移动通信中，普通低速度(120km/h)情况下多普勒效应不明显，但当速度 <span style="color:blue;">超过200km/h的临界速度</span> 时，多普勒效应愈显突出，影响通信效果；

- [x] 高速运动时，电磁波会产生多普勒效应，信号频率会随着速度而改变， <span style="color:red;">信道条件变差，误码率提高</span> ，降低了传输效率。
...
- 名字对应传统传输中的后向纠错(接收、检验、重发)，通过多个冗余位形成纠错码(接收、冗余码检查、纠错)；
- 数据开销较大。

#### **分集技术**

- 将一组信号用多个逻辑信道发送；
- 避免单一信道受干扰后，所有信号全部丢失(TDM、FDM)；
- 信道开销较大。

#### **自适应均衡技术**

- 为 <span style="color:red;">避免多组信号之间的干扰</span> ，先将多组信号拥过编码技术合并，在接收分离；
- 系统开销较大。
...
- [x] 按功能分为信源、编码器、信道、译码器、信宿。

<p align="center">
  <img src="./img/通信模型.png" alt="通信模型">
</p>
...
- 也称 <span style="color:red;">单工</span> ，通信的方向是单向的

<p align="center">
  <img src="./img/通信模型1.png" alt="通信模型">
</p>

### **双向交替通信**

- 也称 <span style="color:red;">半双工</span> ，通信是双向进行的，但不能同时发送和接收

<p align="center">
  <img src="./img/通信模型2.png" alt="通信模型">
</p>

### **双向同时通信**

- 也称 <span style="color:red;">全双工</span> ，通信是双向进行的，可同时发送和接收

<p align="center">
  <img src="./img/通信模型3.png" alt="通信模型">
</p>
...
- `数据(data)` ：涉及事物的形式，定义为传达某种意义或信息的实体，如：语音、图像或ASCII；
- `信号(signal)` ：数据的电气或电磁表示 ；
- `传输(transmission)` ：通过信号的传播和处理进行数据通信的过程。
...
- **编码问题分类**
  - 以提高数字信息传输、存储处理的 `有效性` 为宗旨的信源编码(Source coding)；
  - 以保证数字信息传输和处理的 `可靠性` 为目的的信道编码(channel coding)，又称为差错控制编码(error-control coding)；
  - 以增加数字信息传输、存储的 `安全性` 为目标的数据加密编码(data encryption)。
...
要运一批碗到外地，首先在装箱的时候，将碗摞在一起，这就类似于 <span style="color:red;">信源编码</span> ：压缩以便更加有效率。然后在箱子中的空隙填上报纸、泡沫，做保护，就像 <span style="color:red;">信道编码</span> ：保证可靠。最后在箱子上上锁，避免被他人取走，就类似 <span style="color:red;">数据加密编码</span> ：保证内容安全。

<!-- chat:end -->

### 信号编码准则

- [x] 数字信号是离散的、非连续性的电压脉冲序列，每个脉冲就是一个信号元素，二进制数据传输就是通过把每个数据编码成信号元素完成的；
  - 最简单的情况下，位和信号元素存在一对一的关系，如0代表低电平，1代表高电平；
- [x] 数据率：以bit/s为单位的数据传输的速率
  - 一个位的持续时间长度是指发送器发送这个位所需的时间，如果数据率为R，则位的持续时间为1/R；
- [x] 调制速率：信号电平改变的速率依赖编码特性，调制速率用波特率(Band)来表示，指每秒钟多少个信号元素；
- [x] 位和信号元素 `一对一` 的情况下 `传输率和调制速率相等` 。
...
- [x] 决定接收器能够成功解释所收到信号的因素主要有：信噪比、数据率和带宽： $C=Wlog_2(1+\frac{S}{N}),w$ 为传输带宽
  - 其它因素保持不变的情况下
    - 数据率增加，误码率也会增加；
    - 信噪比增加，则误码率会降低；
    - 增加带宽可以增加数据率。
...
- [x] 编码机制也可以用来改进传输性能
  - 编码机制是一种简单的从数据位到信号元素的 `映射关系`
    - 比如：对信号频谱进行预处理，避免使用高频部分和频带边缘部分传输信号。
...
- [x] 实现多路复用的关键
  - 把多路信号汇合到一条信道上之后，在接收端必须能 `正确地分解` 出各种信号。
...
- [x] 频分多路复用(Frequency Division Multiplexing)
  - 每个数据信号被调制到具有不同频率的载波上，所有的信号在一个信道上同时传送。

<p align="center">
  <img src="./img/频分多路复用.png" alt="频分多路复用">
</p>

##### **时分多路复用**

- [x] 时分多路复用(Time Division Multiplexing)
  - 以 `时间` 作为分割信号的依据；
  - 它利用每个信号在 `时间上交叉` ，可在一个传输通路上传输多个数字信号(或运载数字数据的模拟信号)。

<p align="center">
  <img src="./img/时分多路复用.png" alt="时分多路复用">
</p>

##### **码分多路复用**

- [x] 码分多路复用(Code Division Multiple Access)
  - 每个站使用整个频段发送信号；
  - **多个站的信号可以线性叠加** ；
  - 利用编码技术分离并发的传输。
...
- [x] 码分复用的原理
  - 假设站A、站B、站C同时传输(A发送1，B发送1，C发送0)
  - T站等待接收来自C的报文
  - 站T接收到的信息是三个站发送的信息的叠加结果： $S=A+B+C$
...
<strong style="color:red;">凯撒密码</strong> ：如果手持一张羊皮纸，上面写着乱七八糟的文字，然后如果有一个人拿着一张打着孔的羊皮贴到写字的羊皮上之后，就能复原信息，孔在不同的位置，信息呈现就会有所不同，这是一种数据加密的方式，同时也是一种数据复用的方式。其实这就是 <strong style = "color:purple">码分多址</strong> 。

#### **Tips**

因此码分多址天生就和加密不可分，这种编码方式天生具有安全性，究其深层原因， <strong style="color:blue;">实质在于“码”本身参与了编码</strong> ， <strong style="color:blue;">而码是特定区间内用户唯一的</strong> ， <strong style="color:red;">这个码可以理解成密钥</strong> ， <strong style="color:red;">也可以理解成收发方作为一个整体共享的私钥</strong> 。
...
- [x] Space Division Multiple Access
...
- [x] FDMA：一个房间，AB两人口语对话，CD两人手语对话；
- [x] TDMA：一个房间，分时段，比如，上午AB对话，下午CD对话 ；
- [x] CDMA：一个房间，很多人对话，只是每对人都用不同的语言；
- [x] SDMA：一个房间分成多个小隔间，AB在一个小隔间内对话，CD在另外一个小隔间内对话。

#### OFDM

- `FDMA` ：不同用户的数据在频率上并不是紧密相连的，而是要留出一定的频率间隔，这种间隔被叫做隔离带，或者说是保护带。

<p align="center">
  <img src="./img/FDMA.png" alt="FDMA">
</p>
...
无线电波在频率上很难做到刚好保持在某个频率范围内，或多或少都会出现一些“泄露”，导致用户之间的数据在各自的 <strong style="color:blue;">频率边界上互相干扰</strong> 。

#### **Tips**

因此FDMA系统的频率资源并不能百分之百利用，而是要留一定的频率空间给隔离带，以防止数据在频率上互相干扰，导致性能下降。

<!-- chat:end -->

- [x] Orthogonal Frequency Division Multiplexing，正交频分复用
  - 多载波调制(Multi Carrier Modulation，MCM)的一种，  <span style="color:blue;">通过</span><strong style="color:red;">频分复用</strong><span style="color:blue;">实现高速串行数据的并行传输</span> ；
  - 频分复用的不同用户数据之间不再需要隔离带，而且还可以互相交叠在一起，在接收端能正确区分；
  - 通过将不同用户的数据通过成 <span style="color:blue;">互相正交(积分在一个区间内为0)的载波</span> 来传输，大大提升了频带利用效率；
  - 目前，几乎所有主流商用的无线通信技术都使用了OFDM，包括4G、5G、WiFi。

<p align="center">
  <img src="./img/OFDM.png" alt="OFDM">
</p>

- [x] OFDM中的“O”表示“正交”
  - $sin(t)$ 和 $sin(2t)$ 正交： $sin(t)·sin(2t)$ 在区间 $[0,2π]$ 上的积分为0

<p align="center">
  <img src="./img/OFDM1.png" alt="OFDM">
</p>

- [x] 每个子载波序列都发送自己的信号，在空中相互交叠，最终在接收端看到的信号就是交叠后的信号 $f(t)$ ；
- [x] 接收端收到 $f(t)$ 后，再在每个子载波上分别作 `相乘后积分` 的操作，就可以取出每个子载波分别承载的信号了；

<p align="center">
  <img src="./img/OFDM2.png" alt="OFDM">
</p>
...
- [x] 可以有效地 <span style="color:red;">对抗信号波形间的干扰</span> ，适用于多径环境和衰落信道中的高速数据传输；
- [x] 通过各个子载波的联合编码，具有 <span style="color:red;">很强的抗衰落能力</span> ；
...
- [x] OFDM自适应跳频技术会相应 <span style="color:blue;">增加发射机和接收机的复杂度</span> ，当终端移动速度每小时高于30km时，自适应跳频就不是很适合了

<!-- tabs:end -->

#### OFDMA

- [x] OFDM assigns one block (in time) to <kbd style="color:red">one user</kbd> .
- [x] OFDMA (orthogonal frequency division multiple access, OFDM多址接入技术), which assigns different groups of subcarriers (in frequency) to <kbd style="color:red">different users</kbd> . 
- [x] OFDMA是OFDM技术的演进
  - 在利用OFDM对信道进行子载波化后，在部分子载波上加载传输数据的传输技术；
  - OFDMA多址接入系统将传输带宽划分成正交的互不重叠的一系列子载波集，将不同的子载波集分配给不同的用户实现多址；
  - OFDMA系统可动态地把可用带宽资源分配给需要的用户，实现系统资源的优化利用。
...
OFDM将所有子载波用于承载 <span style="color:blue">一个用户</span> 的数据包，在某一时隙，只有一个用户在该载波上可以发送数据，如果该载波需服务于多个用户，则需排队；

#### **Tips**

OFDMA则可以在同一时隙将不同的子载波分给 <span style="color:blue">多个</span> 用户。

<!-- chat:end -->

#### NOMA

- [x] Non-orthogonal multiple access：非正交多址访问
  - NOMA其前身就是SIC(Successive Interference Cancellation，串行干扰消除)，核心思想是利用无线网络中的 `捕获效应` (当两个以上的信号在相同信道上同时传输时，最强的信号仍可能被成功接收的现象)，对接收信号逐次解调；
  - NOMA的子信道传输依然采用4G中的正交频分复用(OFDM)技术，子信道之间是相互正交的，互不干扰。区别在于其一个子信道的资源不再只分配给一个用户，而是由 `多个用户共享` 。

- [x] NOMA兼顾了CDMA和OFDMA的优点同时也很大程度避免了其缺点，是5G的候选技术之一：
  - 子信道之间采用正交传输，因此不存在明显的远近效应问题，多址干扰(MAI)问题也没那么严重；
  - 由于可以不依赖用户反馈的CSI信息，在采用AMC和功率复用技术后，应对各种多变的链路状态更加自如；
  - 同一子信道上可以由多个用户共享，在保证传输速度的同时，可以提高频谱效率。

## 微波扩频通信技术

- [x] 扩展频谱通信技术(Spread Spectrum Communication)，简称扩频通信技术；
  - 原理是用 <span style="color:blue">宽带传输技术来换取信噪比上的好处</span> ；
  - 传输信息之前，扩展所传信号频谱，利用宽频谱获得 <span style="color:red">较强的抗干扰能力、较高的传输速率</span> ；
  - 由于在相同频带上利用不同码型可以承载不同用户的信息，因此扩频也 <span style="color:red">提高了频带的复用率</span> 。

- [x] 香农公式： $C = Wlog2(1 + SNR)$ 
  - C：传输速度，W：传输带宽，SNR：信噪比；
  - 当SNR降低时，通过适当增加B，可以保持速度C不变；
  - 即： `增加信号带宽可以降低对信噪比的要求`。
...
- [x] Direct Sequence Spread Spectrum，直接序列扩频；
- [x] 用高速率的伪随机序列与信息码序列模二加(波形相乘)后的复合码序列去控制载波的相位而获得直接序列扩频信号，即将原来较高功率、较窄的频率变成具有较宽频的低功率频率，以在无线通信方面获得更好的抗噪声干扰性能；
- [x] 不同速率的信道扩频采用不同的编码，如：1Mbps采用BPSK，2Mbps采用QPSK。

### **FHSS**

- [x] Frequency-Hopping Spread Spectrum，跳频扩频技术；
- [x] 在同步、且同时的情况下，接受两端以特定型式的窄频载波来传送讯号，对于一个非特定的接受器，FHSS所产生的跳动讯号对它只算是脉冲噪声；
- [x] FHSS可以有效降低多路信号同频信号间的干扰，同时可随时根据不同数据传输需求改变信号的传输速率，但在降低功率方面表现一般。
...
- 扩频编码随机生成，同类信号间经扩频后频率也不相同；

#### **屏蔽性强、干扰小**
- 经宽带扩频频后，原窄带信号被散布在宽带中，单位频带功率小，被窃取后不易发现信号存在，低功率对其它设备干扰较小；

#### **易实现多路复用**

- 根据扩频码的不同可以控制每路信息所占用信道位置，可同时在同一频率信道上实现多路复用；

#### **抗多径干扰**

- 利用扩频码技术容易在接收端将多径信号中提取出最强的有用信号，也可以通过多经同一码序列的信号进行相位调整叠加，达到较好的抗多径干扰效果。

#### **安全性**

- 扩频系统将传输的信息扩展到很宽的频带上去，其功率密度随着频谱的展宽而降低，甚至可以将信号淹没在噪音中；
- 因此其保密性很强，要截获或窃听、侦察这样的信号是非常困难的，除非采用与发送端所用的扩频码且与之同步后进行相关的检测。

#### **自由频段**

- 由于扩频信号功率频谱密度很低，在许多国家，如美、日、欧洲等国家对专用频段，如ISM频段，只要功率频谱密度满足一定的要求，就可以不经批准使用频段；
- 在中国，原则上国家无线管理委员会没有允许的自由频段，任何无线频段的使用都要经过申请，但在2.4G~2.483G的频段上，国家无线管理委员会是允许民用产品使用，一般申请都会同意，而且基本上不要费用。
...
<details>
  <summary>5G概述</summary>
  <p>5G是第五代移动通信系统(5th generation mobile/wireless/cellular system)的简称，是4G(LTE/WiMax)之后的新一代移动通信系统。</p>
  <p>5G的性能目标是高数据速率、低延迟、大容量、节能、低成本。</p>
  <p>5G的法定名称是“IMT-2020”</p>
  <p>关键技术包括：大规模天线阵列、超密集组网、新型多址、全频谱接入和新型网络架构；</p>
  <p>1G实现了模拟语音通信，2G实现了语音通信数字化，3G实现了多媒体通信，4G实现了高速上网；</p>
  <p>1G~4G都是着眼于人与人之间更方便快捷的通信，5G将实现随时、随地、万物互联，让人类可以与地球上的万物无时差同步通信。</p>
</details>

### ITU对5G主要移动网络业务分类

- [x] eMBB(Enhanced Mobile Broadband)
  - 聚焦对 `带宽` 有极高需求的业务，满足人们对于数字化生活的需求；
  - 例如：高清视频、虚拟现实/增强现实等；

- [x] uRLLC(Ultra-reliable and Low-latency Communications)
  - 聚焦对时 `延极` 其敏感的业务，满足人们对于数字化工业的需求；
  - 例如：车联网、远程手术、智能制造等；

- [x] mMTC(Massive Machine Type Communications)
  - 覆盖对于 `联接密度` 要求较高的场景，满足人们对于数字化社会的需求；
  - 如：智慧城市、智能农业。

### 5G要求

- [x] 5G系统为了满足更高的峰值速率、低时延、高容量的需求，提出了5G新空口(New Radio，NR)的概念；
- [x] 新空口引入了许多新技术，全面提升了5G网络的性能。
- [x] 为了连接万物，就必须从三个方面对网络进行重新设计和优化：容量、速率、时延。

### 无线频段

- [x] 一个重要公式：c=λ⋅ν
- [x] 即：光速=波长×频率，光速为常量，则频率与波长成反比。

> 频段越高，带宽越大

- [x] 增加带宽的核心方法就是采用更高的频段

> 频段越高，价格越低

- [x] 5G若想更高速，就得使用更大的带宽，而要取得更大的带宽，就得使用更高的频段。
  - 4G之前使用是特高频段，5G就应该向超高频甚至更高的频段发展；
  - 国际电信联盟的专家预测将来有可能使用30GHz~60GHz的频段；
  - 30GHz以上的频段，比超高频还要高，就是波长更短的毫米波。

- [x] 电波传播特性：频率越高(波长越短)损耗越快，绕射和穿透能力越差，反射能力强。
  - 毫米波28GHz由于频率高，每一步的衰减都要比2.6GHz多很多；
  - 同样的发射功率和同样的传播路径，终端用户收到28GHz的信号是2.6GHz信号强度的百万分之一！
  - 关键技术：微基站、MIMO、波束赋形等。
...
- [x] 电磁波有五种传播模式，相对于5G，目前手机的频率要低得多，其绕射能力比较强，因为信号可以绕着到达，建筑物遮挡处的信号受影响不大；
- [x] 5G的频率会高得多，绕射能力会下降，信号直线传输，以往信号能到达的非直线区域就很难覆盖。
  - 频率越高，反射越强，穿透能力越差，覆盖范围越小

- [x] 将传统的宏基站变成 `站点更多` 、 `覆盖面积更小` 、 `密度更大` 的微基站，是解决毫米波“直线传播”的有效方法；
  - 小区中心只立着一盏路灯，阴影面积会很大，而如果在小区里均匀设置很多路灯，阴影面积则会小得多了；

- [x] 5G时代的 `入网设备数量` 会呈爆炸性的增长，单位面积内的入网设备可能会增至千倍，若延续以往的宏基站覆盖模式，即使基站的带宽再大也无力支撑。
- [x] 基站数量越多，辐射反而越小；
- [x] 基站越小，数量越多，覆盖就越好，速度就越快。

> 问题：微基站密度加大，需要避免基站之间的频谱互扰，以及考虑协同工作。

- [x] 为了解决毫米波的覆盖问题，可以毫米波和低频Sub6G `协同组网` ，同时使用两个频段， `低频负责控制面` ， `高频负责用户面` ，这样既可以进行无感知的小区切换，还能享用高频带来的极致速率。

> 虽然毫米波有很多缺点，但为了实现eMBB业务，必须采用各种技术方案来弥补。

#### **高阶MIMO技术**

- [x] 根据天线理论， `天线长度应与波长成正比` ，大约在1/10~1/4之间，当前手机使用的是甚高频段(即分米波)，天线长线大约几厘米；
- [x] 5G的频率在提升几十倍后，相应的手线天线长度也会降低到以前的几十分之一，会变成 `毫米级的微型天线` ，手机里就可以安装多个天线，形成 `多天线阵列` ；( $天线长度 = \frac{波长}{10} \sim \frac{波长}{4}$ )
- [x] 基站与手机之间有很多对信道并行通信，每一对天线都独立传送一路信息，汇集后可成倍提高速率。
  - 提高终端接收信号强度，避免信号干扰；
  - 同时同频服务更多用户，提高网络容量；
  - 更好地实现信号覆盖。
...
> - 目前的基站不知道设备在哪个方位，所以使用的电磁波是全向辐射的
> - 如同电灯泡发出的光，只有到达手机的辐射才是有用的，其它方向的辐射都是浪费的；
> - 并且这种巨大的无用辐射还成为了其它手机的干扰；
> - 因为手电筒的能量更集中，所以比灯泡照的更远，基站与某部手机的关系就相当于光源与被照射物的关系。
...
- [x] 在目前的通信网络中，即使是两个人面对面拨打对方的手机(或手机对传照片)，信号都是通过基站进行中转的，包括控制信令和数据包；
- [x] 5G时代，同一基站下的两个用户，如果互相进行通信，他们的数据将不再通过基站转发，而是直接手机到手机；
- [x] 这样就节约了大量的空中资源，也减轻了基站的压力。
  - 控制信令还是要从基站走，而且用着频谱资源，还得付费。

#### **多址接入技术**

- 1G：FDMA，容易受噪声的干扰，语音质量较差；
- 2G：TDMA，使得通信系统向宽带化发展，语音质量得到了较大的改善。其中欧洲制定的GSM系统非常成功，至今仍在广泛使用；
- 3G：CDMA，但是CDMA存在自干扰问题，其频谱效率只比GSM高10%左右，并且3G的主流标准WCDMA的系统设计过于复杂，导致部署成本比较高，所以一直无法替代GSM系统；
- 4G：OFDM，从根本上克服了CDMA的技术缺陷，并且简化了系统设计，成就了一代成功的移动通信系统；
- 5G：NOMA(Non-orthogonal Multiple Access，非正交多址接入)
  - 基本思想是在发送端采用非正交发送，主动引入干扰信息，在接收端通过串行干扰删除(SIC)接收机实现正确解调；
  - 采用SIC技术的接收机复杂度有一定的提高，但是可以提高频谱效率；
  - 本质是用提高接收机的复杂度来换取频谱效率。
...
NOMA面临的难题：非正交传输的接收机相当复杂，需要提高 <span style="color:red">信号处理芯片</span> 技术； <span style="color:red">功率复用</span> 技术还不是很成熟。

<!-- chat:end -->

#### **空口智能、灵活双工技术**

- 2G、3G和4G网络中主要采用两种双工方式，即频分FDD和时分双工TDD，且每个网络只能用一种双工模式；
- 由于5G网络要支持不同的场景和多种业务，并且考虑业务的动态性，因此 <span style="color:red">需要5G系统能根据不同的需求，能灵活智能的使用FDD/TDD双工方式</span> ，发挥各自优势，全面提升网络性能；
- 5G网络把FDD和TDD紧密结合在一起，通过对业务和环境的感知， <span style="color:blue">智能地调整和使用双工模式</span> ，根据上下行业务变化情况动态分配上下行资源，有效提高系统资源利用率。

<!-- tabs:start -->

##### **同频同时全双工技术**

- **终端和网络之间的上下行链路使用相同的频率 `同时` 传输数据**；
- 理论上讲，同时同频全双工可提升一倍的频谱效率，但是由于上下行链路是用同一频率同时传输信号，因而存在严重的 `自干扰` 问题，需要在设备研发和网络部署时严格控制自干扰问题；
- 基于 `自干扰抑制` 的同时同频全双工技术(CCFD)是实现同频同时全双工的有效解决方案。

<p align="center">
  <img src="./img/同频同时全双工技术.png" alt="同频同时全双工技术">
</p>
...
- [x] 目前呼声最高是采用华为提出的F-OFDM技术：F-OFDM采用可配置的灵活的子载波带宽、符号长度以及循环前缀长度，以满足超大带宽、超低时延和超大规模连接三大应用场景的需求。


#### **多址技术**

- [x] 4G的多址接入方式为OFDMA，5G的eMBB场景的多址接入方式仍然基于OFDMA；
- [x] 但对于mMTC大连接场景，改进的OFDMA可能无法满足需求，因此3GPP RAN1在2016年中的会议决定：
  - [x] eMBB场景的多址接入方式应基于正交的多址方式(OFDMA)；
  - [x] 非正交的多址(NOMA)技术将用于mMTC 的上行场景。
